...
→ **範囲を絞らない。問題が参照する図表はすべて再現する**（ユーザ指定：完璧な再現を追求する）。「この図は難しいから省く」という判断はしない。
→ 着手前に PDF を画像化して**図表の棚卸し一覧（図番号・何を表す図か・設問との対応）をユーザに報告**する。これは範囲を相談するためではなく、**作業量と進捗を共有するため**。報告したらそのまま全図に着手してよい。
→ 再現の水準: **ノード・リンク・ラベル・番号・凡例・注記まで原図と同じ情報を落とさない**。レイアウトは375px で読めるよう再構成してよいが、**情報の省略はしない**。図中の記号（(a) (b) など設問が参照する箇所）は原図と同じ記号・同じ位置関係にする。
...
```tsx
{
  ...
  title: '午後解説（開発中）',
  description: '',                 // ← 説明文は書かない（ユーザ指定）
  iconBg: 'bg-teal-50',
  icon: <なにかアイコン className="w-6 h-6 text-teal-600" />,
}
```

- **名称は「午後解説（開発中）」で固定**（ユーザ指定）。勝手に「午後Ⅰ解説」等に変えない。
- **`description` は空文字にする**（ユーザ指定：説明文は書かない）。`MenuCard` の `description` は必須プロパティなので、**型は変えず `''` を渡す**のが最小変更。
  `MenuCardGrid` は `<p className="... truncate">{card.description}</p>` を無条件に描くが、空文字なら行ボックスが生まれず高さ0になる。カードは `items-center` なので**タイトルが縦中央に来て見た目は破綻しない**。実機で確認すること。
  もし空の `<p>` が気になるなら `{card.description && <p>...}` に変える手もあるが、**それは全カード共通の描画を触ることになる**ので、原則やらない。
- 配置は「その他機能」セクション内。**末尾に追加**するのが無難（既存カードの順序を崩さない）。
- `iconBg`／アイコンは既存4枚の書式に合わせる。色は既存と重複しないものを選ぶ（既存は red / orange / purple / blue）。
...
方針はすべて §0 で確定済み。**質問で止まらず、§6-P0 から着手してよい。**

確定事項（再掲）: G1のみ ／ R6-G1-1 の1問だけ ／ 独立構成（既存変更は `App.tsx` と `Home.tsx` の追記のみ） ／ 同期対象外 ／ 公式設問文は原文転記 ／ 図表は完璧に再現（範囲を絞らない） ／ 問題本文は PDF 前提 ／ 記録・XP・バッジなし ／ 入口は「その他機能」の「午後解説（開発中）」カード（説明文なし）。

勝手に判断しないこと:
- 上記を「たぶんこうだろう」で緩めない。迷ったら**緩める側ではなく、書いてあるとおりに**作る。
- 仕様の変更が必要だと判断した場合だけ、**理由を添えてユーザに確認**する。
