...
- 問5: EVM 表は次工程の図表化対象とし、選択肢 `110 / 120 / 135 / 150` と正解 `エ` を確認した。
- 問6: `HP－ABP`、`Highly Possible`、`Aggressive But Possible`、プロジェクトバッファ条件を原画像で確認した。
- 問7: PDM 図とアクティビティ表は次工程の図表化対象とし、本文の `同じ専門チーム` を原画像で確認した。
- 問8: `合流バッファ`、`プロジェクトバッファ`、`クリティカルパス` を補正した。
- 問10: FP 表1〜表4は次工程の図表化対象とし、`JIS X 0142:2010` と `IFPUG 4.1版` を確認した。
- 問13: レビュー手法の組合せ表は、選択肢として `a/b/c` の対応を1行化した。次工程で table 化候補。
- 問18: TCO の4案表は次工程の図表化対象とし、選択肢 `A案`〜`D案` と正解 `ウ` を確認した。
...
上記は OCR フェーズでは `figure` を生成せず、`categoryId` に `(figure)` suffix を付与した。SVG/table 化は次工程 Claude 担当。
