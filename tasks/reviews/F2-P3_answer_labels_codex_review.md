...
## 追加対応: 選択肢ランダム化 OFF モード
- `/morning` に「選択肢表示」設定を追加し、`選択肢ランダム` / `公式順` を選択可能にした。
- 設定は端末ローカル preference として保存する。
- `選択肢ランダム`: 従来どおり選択肢をシャッフルし、出題中は `①②③④`、解答後に `解答記号：ア/イ/ウ/エ` を補助表示する。
- `公式順`: 選択肢をシャッフルせず、出題中から `ア/イ/ウ/エ` で表示する。解答後の `解答記号：` 補助行は表示しない。
- ブラウザ確認:
  - 公式順 H27 問12: 解答前から `ア.感度分析` / `イ.期待金額価値分析` / `ウ.デシジョンツリー分析` / `エ.発生確率・影響度マトリックス`、解答後も補助行なし。
  - ランダム H27 問8: 解答前は `①②③④`、解答後は `解答記号：` 補助行が4件表示。
