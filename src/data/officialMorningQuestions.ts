...
export const officialMorningQuestions: OfficialMorningQuestion[] = [
  {
    ...
    correctIndex: 1,
    explanation: 'プロジェクトマネジメント計画書は，プロジェクトの実行・監視・管理の方法を定義し，スコープ・スケジュール・コストなどのベースラインを統合した文書である。\nア はビジネスケース，\nウ はプロジェクトスコープ記述書，\nエ はプロジェクト憲章の説明。',
    categoryId: 'planning',
  ...
  },
  {
    ...
    correctIndex: 0,
    explanation: 'RACI チャートは責任分担マトリックスの一形式で，R=Responsible（実行責任），A=Accountable（説明責任），C=Consulted（相談対応），I=Informed（情報提供）の 4 役割を割り当てる。\nイ・ウ・エ はいずれも「リスク管理」を含むが，RACI に「リスク管理」という役割は存在しないため誤り。',
    categoryId: 'team',
    sourceUrl: R6_AUTUMN_PM_AM2_SOURCE_URL,
  },
  {
    ...
    correctIndex: 2,
    explanation: 'JIS Q 21500:2018「プロジェクト組織の定義」プロセスは，役割・責任・権限について すべての当事者から必要なコミットメントを得ることが目的。\nア は「アクティビティ資源の見積り」，\nイ は「ステークホルダーの特定」，\nエ は「プロジェクトチームの編成」プロセスの目的。',
    categoryId: 'team',
    sourceUrl: R6_AUTUMN_PM_AM2_SOURCE_URL,
  },
  {
    ...
    correctIndex: 2,
    explanation: 'TCPI（残作業効率指数）は BAC どおりに完了させるため，残予算で残作業を消化するのに必要なコスト効率指数で，TCPI = (BAC − EV) / (BAC − AC) で算出する。\n代入: TCPI = (100 − 34) / (100 − 40) = 66 / 60 = 1.1。\n1.0 を超えるため，これまで以上のコスト効率が今後求められることを意味する。',
  ...
  },
  {
    ...
    choices: ['12', '14', '18', '21'],
    correctIndex: 3,
    explanation: 'クリティカルパスは A(8)+D(10)+G(12) = 30 日。\nH の後続作業は I(4) のみで，H と I の所要は 5+4 = 9 日。\n30 日以内に完了させるため H の最遅開始は 30 − 9 = 21 日後となる。\nア〜ウは A→D 経由のクリティカル制約を満たさず，全体 30 日を超えてしまう。',
    categoryId: 'planning',
    sourceUrl: R6_AUTUMN_PM_AM2_SOURCE_URL,
    figure: {
      type: 'svg',
  ...
  },
  {
    ...
    choices: ['18', '19', '20', '21'],
    correctIndex: 1,
    explanation: 'PDM 上の FS 依存に従う最早完了は A(8)+C(5)+D(5) = 18 日，B = 12 日でクリティカルパスは 18 日。\nただし試験設備は 1 台で，各アクティビティの「最終 3 日間」が競合する。\n設備使用区間が重ならないように 1 日ずらすと最少所要日数は 19 日（イ）となる。\nア 18 日は資源制約を無視した値，ウ・エ は過剰なシフトで最適でない。',
    categoryId: 'planning',
    sourceUrl: R6_AUTUMN_PM_AM2_SOURCE_URL,
    figure: {
      type: 'svg',
  ...
  },
  {
    ...
    correctIndex: 3,
    explanation: 'ベロシティは「定められた期間（1 スプリント）に受入が完了したチームの成果物の量」であり，チームの実績生産性を計測する指標。\nア はストーリーポイント，\nイ はプロダクトバックログ，\nウ はバーンダウンチャートの説明。',
  ...
  },
  {
    ...
    correctIndex: 2,
    explanation: 'CPI<1.0 はコストが超過している状態。原因を分析し是正処置を実施したうえで CPI を継続監視するのが定石。\nア は逆方向の修正，\nイ は値が完成時総コスト見積りとして妥当でない計算，\nエ は CPI が自然に 1.0 になる保証がないため不適切。',
    categoryId: 'measurement',
    sourceUrl: R6_AUTUMN_PM_AM2_SOURCE_URL,
  },
  {
    ...
    correctIndex: 0,
    explanation: 'E = 5.2 × 10^{0.98} ≒ 5.2 × 9.55 ≒ 49.66 人月。\n生産性 = L / E = 10 / 49.66 ≒ 0.20 k ステップ/人月。\n指数が 1 に近いためほぼ線形だが，0.98 の影響でわずかに規模増加の効率が落ちている。',
    categoryId: 'measurement',
    sourceUrl: R6_AUTUMN_PM_AM2_SOURCE_URL,
  },
  {
    ...
    correctIndex: 3,
    explanation: '直列の工程を 1 人月で通過させると考えると，各工程で 1/X，1/Y，1/Z 人月を消費するため，合計所要人月は 1/X + 1/Y + 1/Z。\nよって全体の生産性（ステップ/人月）はこの逆数 1 / (1/X + 1/Y + 1/Z) となる。\nア は単純加算で次元が合わず，イ は単純平均で工程直列に非整合，ウ は逆数の和で生産性そのものを表していない。',
    categoryId: 'measurement',
    sourceUrl: R6_AUTUMN_PM_AM2_SOURCE_URL,
  },
  {
    ...
    correctIndex: 1,
    explanation: '「リスクへの対応」（Treat Risks）プロセスは，特定・評価したリスクに対し予算・スケジュール・資源・活動を 投入して対処する実行プロセス。\nア は「リスクの管理」，\nウ は「リスクの特定」，\nエ は「リスクの評価」プロセスの説明。',
  ...
  },
  {
    ...
    correctIndex: 1,
    explanation: '月別必要 PC 台数は要員計画の合計欄から導かれる（最大は 6 月／7 月／9 月の 11 台）。\nセットアップ 2 週間・データ消去 1 週間がレンタル期間に含まれること，月途中の解約や日割り精算ができないことを考慮して必要月数を計算し，月額 5 千円/台で総額を算出すると最低 470 千円（イ）となる。',
  ...
  },
  {
    ...
    correctIndex: 1,
    explanation: 'GoF（Gang of Four）デザインパターンはオブジェクト指向設計のための 23 パターンで，Creational（生成）・Structural（構造）・Behavioral（振る舞い）の 3 カテゴリに分類される。\nア は Java EE のパターン体系，\nウ は POSA（Pattern-Oriented Software Architecture）の 4 分類，\nエ は Layers / Broker などのアーキテクチャパターンの説明で，いずれも GoF ではない。',
  ...
  },
  {
    ...
    correctIndex: 0,
    explanation: '移行リハーサルの目的は本番移行のリスク洗い出しと手順検証。発生問題は原因究明と本番非発生の確認が完了条件であり，\nア が適切。\nイ は切戻し検証を省略，\nウ は計画外の作業順変更，\nエ は時間超過が放置されており，いずれも目的未達。',
  ...
  },
  {
    ...
    correctIndex: 1,
    explanation: 'CMMI V2.0 成熟度レベルは 1=Initial / 2=Managed / 3=Defined / 4=Quantitatively Managed / 5=Optimizing。レベル 4 は「定量的な実績目標とともにデータで運営される」段階。\nア はレベル 5，\nウ はレベル 3，\nエ はレベル 2 の説明。',
  ...
  },
  {
    ...
    correctIndex: 1,
    explanation: 'INVEST は I=Independent，N=Negotiable，V=Valuable（顧客にとって価値），E=Estimable，S=Small（作業期間に対し適切な大きさ），T=Testable の頭字語。\nイ は S に対応する。\nア は顧客ではなく開発者の価値，\nウ は N に反し，\nエ は I に反する。',
    categoryId: 'development-approach',
    sourceUrl: R6_AUTUMN_PM_AM2_SOURCE_URL,
  },
  {
    ...
    correctIndex: 3,
    explanation: 'アジャイルソフトウェア開発宣言の四つの価値観は「プロセスやツールよりも個人と対話を」「包括的なドキュメントよりも動くソフトウェアを」「契約交渉よりも顧客との協調を」「計画に従うことよりも変化への対応を」。\nエ は「包括的なドキュメントよりも動くソフトウェアを」に一致し正解。\nア は右辺が宣言では「変化への対応」だが「自己組織化されたチームによる裁量」に改変されており誤り。\nイ は右辺が宣言では「顧客との協調」だが「顧客の競争力と満足度の向上」に改変されており誤り。\nウ は右辺が宣言では「個人と対話」だが「実用的なプラクティス」に改変されており誤り。',
    categoryId: 'development-approach',
    sourceUrl: R6_AUTUMN_PM_AM2_SOURCE_URL,
  },
  {
    ...
    correctIndex: 1,
    explanation: 'ISO/IEC 20000-1（JIS Q 20000-1:2020）はサービスマネジメントシステム（SMS）の要求事項を規定する規格で，組織が SMS を確立・実施・維持・継続的に改善するための要件を定める。\nア は製品／ツール仕様で SMS 規格の対象外，\nウ は組織形態に依存しない汎用要求のため「組織の形態・規模に応じた要求」は誤り，\nエ は用語の置き換え（変換）ではなく規格自体に用語定義が含まれているため誤り。',
  ...
  },
  {
    ...
    correctIndex: 0,
    explanation: 'フルバックアップ間隔を 2 倍にすると，前回バックアップから現時点までのログ蓄積量が約 2 倍となり，復旧時に行うログ反映（ロールフォワード）の処理時間が約 2 倍に増える。\nイ・ウ バックアップ 1 回当たりの容量はデータベース総量で決まり間隔には依存しないため変化しない，\nエ フルバックアップ取得時間も総量依存で間隔とは無関係のため変化しない。',
    categoryId: 'service-management',
    sourceUrl: R6_AUTUMN_PM_AM2_SOURCE_URL,
  },
  {
    ...
    correctIndex: 3,
    explanation: '実費償還契約（Cost Reimbursable）は，ベンダーが負担した実費に報酬（フィー）を加えて支払う契約形態。\nア は完全定額契約（FFP），\nイ は価格調整付き定額契約，\nウ はインセンティブフィー（CPIF）の説明。',
    categoryId: 'project-work',
    sourceUrl: R6_AUTUMN_PM_AM2_SOURCE_URL,
  },
  {
    ...
    correctIndex: 2,
    explanation: '経済産業省「AI・データの利用に関する契約ガイドライン」は法的拘束力をもたず，AI 開発・利用の契約プラクティスが未確立であることを背景に，参考枠組みを提示するもの。\nア は法律で全て規定されている前提が誤り，\nイ はデータの経済価値や秘匿性は考慮必須，\nエ はガイドラインに法的拘束力なしのため誤り。',
    categoryId: 'project-work',
    sourceUrl: R6_AUTUMN_PM_AM2_SOURCE_URL,
  },
  {
    ...
    correctIndex: 1,
    explanation: '「心の警備（Mindguard）」は，集団思考を脅かす情報や異議を遮断して集団を保護する役割を指す。\nア は「不死身の幻想」，\nウ は「全会一致の幻想」，\nエ は「異議者への直接圧力」で，ジャニスの集団思考 8 兆候の別項目に該当する。',
    categoryId: 'governance',
    sourceUrl: R6_AUTUMN_PM_AM2_SOURCE_URL,
  },
  {
    ...
    correctIndex: 3,
    explanation: 'SAML（Security Assertion Markup Language）は，ID プロバイダとサービスプロバイダの間で認証・認可に関する情報を XML 形式で交換する仕様で，シングルサインオン（SSO）の基盤として広く利用される。\nア は SNMP 等の監視プロトコル，\nイ は STIX/TAXII などの脅威情報共有仕様，\nウ は IPsec／TLS による VPN 仕様の説明で，いずれも SAML の機能ではない。',
    categoryId: 'service-management',
    sourceUrl: R6_AUTUMN_PM_AM2_SOURCE_URL,
  },
  {
    ...
    correctIndex: 3,
    explanation: 'JPCERT/CC「CSIRT ガイド」のインシデントハンドリングは検知 → トリアージ → 対応 → 報告の流れで構成される。トリアージは緊急度・影響度を評価し対応優先順位を決める活動で，ハンドリングの中核。ア・イ は事前準備，\nウ は別業務（脆弱性ハンドリング）。',
    categoryId: 'service-management',
    sourceUrl: R6_AUTUMN_PM_AM2_SOURCE_URL,
  },
  {
    ...
    correctIndex: 2,
    explanation: '公開 PoC は既知の脆弱性を悪用する。最も効果的な未然防止策は，使用ソフトウェアの脆弱性情報を確認し，修正プログラムを適用（または一時的なワークアラウンドを実施）すること。\nア は外部攻撃と直接関係せず，\nイ は事後検知，\nエ は侵害後の検知策で予防策ではない。',
    categoryId: 'service-management',
    sourceUrl: R6_AUTUMN_PM_AM2_SOURCE_URL,
  },
  {
