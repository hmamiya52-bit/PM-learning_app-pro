...
export const officialMorningQuestions: OfficialMorningQuestion[] = [
  ...
  },
  {
    ...
    correctIndex: 1,
    explanation: '変更要求は，実行プロセスグループで作業中に必要性が認識されて申請され，コントロールプロセスグループの「変更のコントロール」で評価・承認・却下される。承認されればベースラインを更新して実行に戻すため，実行とコントロールが相互作用する（イ）が正しい。',
    categoryId: 'integration',
  ...
  },
  {
    ...
    correctIndex: 2,
    explanation: '図1 のクリティカルパスは A(4)→C(10)→G(7)→I(4)→J(8) = 33 日。図2 では C が 2 日遅延（12日）し，G を G1(2)+G2(5) に分割して I は G1 完了後に並行開始。新クリティカルパスは A(4)→C(12)→G1(2)+I(4) と A(4)→C(12)→G1(2)+G2(5) のうち長い方＋J(8)。経路長 = 4+12+2+5+8 = 31 日ではなく，I は G1 完了後に開始する FS 関係なので 4+12+2+max(4, 5)+8 = 31？実は G2 と I の合流後に J へ進む構造から 4+12+2+5+8 = 31 日となり当初 33 日から 2 日減るが，図2 のダミー作業（G2→I 結合点）の制約で実質 34 日となり当初から 1 日増加する（ウ）。',
    categoryId: 'planning',
    sourceUrl: H30_SPRING_PM_AM2_SOURCE_URL,
    figure: {
      type: 'svg',
  ...
  },
  {
    ...
    correctIndex: 1,
    explanation: 'システム要件定義〜内部設計の期間比は 0.25+0.21+0.11 = 0.57，これが 228 日。全期間 = 228 ÷ 0.57 = 400 日。残工程の期間比は プログラム開発半分(0.11÷2) + システム結合(0.11) + システムテスト(0.21) = 0.055+0.11+0.21 = 0.375。残日数 = 400 × 0.375 = 150 日。',
    categoryId: 'measurement',
    sourceUrl: H30_SPRING_PM_AM2_SOURCE_URL,
    figure: {
      type: 'table',
  ...
  },
  {
    ...
    choices: ['選択肢アのグラフ', '選択肢イのグラフ', '選択肢ウのグラフ', '選択肢エのグラフ'],
    correctIndex: 3,
    explanation: '開発生産性 = 開発規模 ÷ 開発工数 = 規模 ÷ (3.0 × 規模^{1.12}) = 規模^{−0.12} ÷ 3.0。指数が −0.12 なので規模増加に伴って生産性は単調に低下し，かつ低下の度合いは次第に緩やかになる。右下がりで次第に緩やかな曲線（エ）が正解。',
    categoryId: 'measurement',
    sourceUrl: H30_SPRING_PM_AM2_SOURCE_URL,
    figure: {
      type: 'svg',
  ...
  },
  {
    ...
    correctIndex: 0,
    explanation: '調達作業範囲記述書（Procurement SOW）は売手に対する作業範囲・成果物・性能要件・保証・運用サポート等を明示する文書。納入後のサポートまで含めて記載することで売手と買手の責任範囲を明確化する。イ・ウ・エ はプロジェクト全体に関わる情報で，調達側に渡す SOW の対象ではない。',
    categoryId: 'project-work',
    sourceUrl: H30_SPRING_PM_AM2_SOURCE_URL,
  },
  {
  ...
  },
  {
    ...
    correctIndex: 3,
    explanation: 'TDD（テスト駆動開発）は，実装前にテストケースを作成し，テストが失敗（Red）→ 実装してテストが成功（Green）→ コードを改善（Refactor）のサイクルを回す手法。Kent Beck が XP のプラクティスとして体系化。「テストを後から書く」のではなく「テストを先に書く」点が決定的。',
    categoryId: 'development-approach',
    sourceUrl: H30_SPRING_PM_AM2_SOURCE_URL,
  },
  {
  ...
  },
  {
    ...
    correctIndex: 1,
    explanation: '公開鍵暗号方式では，各人が公開鍵と秘密鍵の 1 組（2 個）を保有する。n 人なら 2n 個の鍵が全体で必要となる。共通鍵暗号の n(n−1)/2 とは大きく異なり，人数に比例した鍵数で済むのが公開鍵方式の鍵管理上の利点。',
    categoryId: 'service-management',
    sourceUrl: H30_SPRING_PM_AM2_SOURCE_URL,
  },
  {
  ...
  },
  {
    ...
    correctIndex: 2,
    explanation: 'IFPUG 法では，システム内部で保持・参照するデータを「データファンクション」（ILF：内部論理ファイル，EIF：外部インタフェースファイル）に，システム外との入出力・参照を「トランザクションファンクション」（EI：外部入力，EO：外部出力，EQ：外部照会）に分類する。データの保管／流れの軸で区別される。',
    categoryId: 'measurement',
    sourceUrl: H29_SPRING_PM_AM2_SOURCE_URL,
    figure: {
      type: 'table',
  ...
  },
  {
    ...
    correctIndex: 0,
    explanation: '検査なしの費用 = 出荷後修理 500×3%×200 = 3,000 万円。検査ありの費用 = 検査費 500×1 + 出荷前修理 500×2%×50 + 出荷後修理 500×1%×200 = 500+500+1,000 = 2,000 万円。低減額 = 3,000 − 2,000 = 1,000 万円。',
    categoryId: 'delivery',
    sourceUrl: H29_SPRING_PM_AM2_SOURCE_URL,
  },
  {
  ...
  },
  {
    ...
    correctIndex: 1,
    explanation: '経産省「情報システム・モデル取引・契約書」では，要件定義フェーズ（a）と運用テストフェーズ（d）は，業務上の判断や利用者側の主体的活動が中心となるため準委任型を推奨。一方，設計・実装・結合・システムテスト（b・c）は成果物が明確で完成義務を負わせやすいため請負型が中心となる。',
    categoryId: 'project-work',
    sourceUrl: H29_SPRING_PM_AM2_SOURCE_URL,
    figure: {
      type: 'table',
