 ...
 * 公式午前Ⅱ（PM 午前Ⅱ）問題データ
 *
 * 設計書 v0.16 §2.7b / F1.5-P4、および F2-P3 に従い、PM 午前Ⅱ問題を投入。
 ...
 * - 教育目的の引用は許諾・使用料不要
 * - 出典明記必須: 「出典：<年度> <期> プロジェクトマネージャ試験 午前Ⅱ 問<番号>」
...
export const officialMorningQuestions: OfficialMorningQuestion[] = [
  ...
  },
  {
    ...
    correctIndex: 1,
    explanation: '==汎化==（generalization）とは，==複数のクラスに共通する性質を抽出して上位のクラスとして定義する関係== で，下位（具象クラス）は上位（抽象クラス）の特化として位置付けられる（イ）。\n__ア__ は逆方向の ==「特化」==（specialization）。\n__ウ__ は所有関係である ==「集約」==（aggregation）。\n__エ__ は同名メソッドを束ねる ==「ポリモーフィズム」==（インタフェース抽象化）の説明。',
    categoryId: 'tailoring-models',
    sourceUrl: R5_AUTUMN_PM_AM2_SOURCE_URL,
  },
  {
  ...
  },
  {
    ...
    correctIndex: 1,
    explanation: '==プロバイダ責任制限法==（同法 4 条）は，権利侵害情報の流通に対する送信防止措置を講じた場合に，==一定の条件（「権利侵害情報が流通していると信ずるに足りる相当の理由がある」等）を満たせば，特定電気通信役務提供者は発信者への損害賠償責任を負わない== と定める（イ）。\n__ア__ 事前承諾を必須化しており同法の趣旨に反する。\n__ウ__ 裁判所決定の要件は同法に存在しない。\n__エ__ 個人情報保護委員会ではなく==被害者からの直接の申出==に対応する仕組み。',
    categoryId: 'governance',
    sourceUrl: R5_AUTUMN_PM_AM2_SOURCE_URL,
  },
  {
  ...
  },
  {
    ...
    correctIndex: 3,
    explanation: '==WBS==（Work Breakdown Structure）は ==プロジェクトのスコープを階層的に詳細化し，管理可能な大きさのワークパッケージへ細分化== するための ==成果物指向の分解構造==（エ）。\n__ア__ は ==CPM/PERT==（コスト−期間トレードオフ最適化），\n__イ__ は ==ネットワーク図（PDM）==（作業順序とクリティカルパス把握），\n__ウ__ は ==ガントチャート==（横棒で日程と進捗を表示）の説明。',
    categoryId: 'planning',
    sourceUrl: R4_AUTUMN_PM_AM2_SOURCE_URL,
  },
  {
  ...
  },
  {
    ...
    correctIndex: 2,
    explanation: '==SOA==（Service Oriented Architecture）は ==サービス間を疎結合に設計== し，個々のサービスの変更が他に波及しないようにすることで，==業務変化への追随性== を高める（ウ）。\n__ア__ は可用性向上のため ==ステートレス== にすべき（ステートフルは拡張性低下）。\n__イ__ は業務概念に対応した ==具体名== にして使いやすくする（抽象的では不適切）。\n__エ__ は SOA の本質である ==再利用性== を否定しており不適切。',
    categoryId: 'development-approach',
    sourceUrl: R2_OCTOBER_PM_AM2_SOURCE_URL,
  },
  {
  ...
  },
  {
    ...
    correctIndex: 3,
    explanation: '==リバースプロキシ方式の SSO==では，==すべての認証をリバースプロキシで一元化== するため，バックエンド Web サーバの認証手段に依存せず ==デジタル証明書認証== など強固な認証手段を利用できる（エ）。\n__ア__ ==cookie== はサーバが発行し，クライアントが保存する（クライアント生成ではない）。\n__イ__ cookie 方式は ==同一インターネットドメインに配置が必要==（cookie のドメインスコープ制約）。\n__ウ__ リバースプロキシ方式は単一の入口に認証を集約するため，==バックエンドが同一ドメインでも問題なく動作==。',
    categoryId: 'service-management',
    sourceUrl: R2_OCTOBER_PM_AM2_SOURCE_URL,
  },
  {
  ...
  },
  {
    ...
    correctIndex: 1,
    explanation: '==ファジング==は，ソフトウェアの ==データの入出力に注目== し，==問題を引き起こしそうな大量かつ多様なパターンのデータを入力== して挙動を観察し，例外的・想定外の振る舞いから脆弱性を見つける ==動的な検査手法==（イ）。\n__ア__ は ==パッチ管理・バージョン管理== の検査。\n__ウ__ は ==セキュリティアドバイザリ照合==（情報ベース検査）。\n__エ__ は ==静的解析==（ホワイトボックス検査）。',
    categoryId: 'service-management',
    sourceUrl: R2_OCTOBER_PM_AM2_SOURCE_URL,
  },
  {
  ...
  },
  {
    ...
    correctIndex: 1,
    explanation: '==RFI==（Request for Information）は，システム化に当たって ==利用可能な技術・製品やベンダの導入実績などの情報提供を依頼== する文書（イ）。発注前の情報収集段階で用いる。\n__ア__ は ==SLA==（Service Level Agreement，サービス品質保証契約）の説明。\n__ウ__ は ==RFP==（Request for Proposal，提案依頼書）の説明。\n__エ__ は ==業務処理概要・入出力情報をまとめた要件定義書／業務フロー資料== の説明。',
    categoryId: 'project-work',
    sourceUrl: R1_SPRING_PM_AM2_SOURCE_URL,
  },
  {
  ...
  },
  {
    ...
    correctIndex: 3,
    explanation: '==プロジェクト憲章==は __プロジェクトを正式に承認する文書__ で，プロジェクトマネージャを特定して権限を付与し，==ビジネスニーズ・目標・期待される結果== などを明確にする（エ）。\n__ア__ は ==プロジェクト・スコープ記述書==（プロダクト特性）の説明。\n__イ__ は ==プロジェクトマネジメント計画書==（実施・監視・コントロールの方法）の説明。\n__ウ__ も ==スコープ記述書==（最終状態の定義）の説明。',
    categoryId: 'integration',
    sourceUrl: H30_SPRING_PM_AM2_SOURCE_URL,
  },
  {
    ...
    correctIndex: 1,
    explanation: '==コンフィギュレーション・マネジメント==（構成管理）は，==成果物の構成要素や変更履歴を一元管理== し，==要求事項への適合性を監査するための情報基盤== を提供する（イ）。\n__ア__ ==EVM== はコスト・スケジュール統合管理。\n__ウ__ ==コンフリクト・マネジメント== は対立解決の技法。\n__エ__ ==ポートフォリオマネジメント== は複数 PJ の戦略的統合管理で別概念。',
    categoryId: 'integration',
    sourceUrl: H30_SPRING_PM_AM2_SOURCE_URL,
  },
  {
  ...
  },
  {
    ...
    correctIndex: 2,
    explanation: '==ISO 21500:2012==「==資源のコントロール==」プロセスは，==プロジェクト作業の実施に必要な資源を確保== し，==要求事項を満たすように資源を配分== することが目的（ウ）。\n__ア__ は「==アクティビティ資源の見積り==」の目的。\n__イ__ は「==プロジェクトチームの育成==」の目的。\n__エ__ は「==プロジェクトチームの編成==」の目的。',
    categoryId: 'team',
    sourceUrl: H30_SPRING_PM_AM2_SOURCE_URL,
  },
  {
  ...
  },
  {
    ...
    correctIndex: 3,
    explanation: '==発生確率・影響度マトリックス== はリスクの ==「確率」と「影響度」を 2 軸で評価== する ==定性的リスク分析== の代表手法（エ）。\n__ア__ ==感度分析== は ==定量的リスク分析== の手法。\n__イ__ ==EMV（期待金額価値）== も ==定量的==。\n__ウ__ ==デシジョン・ツリー== も ==定量的==（数値的な影響を計算）。',
    categoryId: 'uncertainty',
    sourceUrl: H30_SPRING_PM_AM2_SOURCE_URL,
  },
  {
  ...
  },
  {
    ...
    correctIndex: 3,
    explanation: '==共通フレーム 2013== の「==システム適格性確認テスト==」は，==システム要件の実装適合性をテスト== し，==システム全体として納入準備が整っているかを評価== するアクティビティ（エ）。\n__ア__ は「==運用テスト==」（実運用環境または擬似運用環境での評価）。\n__イ__ は「==業務継続性評価==」。\n__ウ__ は「==投資効果評価==」（業務運用後のレビュー段階）の説明。',
    categoryId: 'delivery',
    sourceUrl: H30_SPRING_PM_AM2_SOURCE_URL,
  },
  {
  ...
  },
  {
    ...
    correctIndex: 0,
    explanation: '委託先のセキュリティ管理では，==契約でセキュリティレベル・遵守事項・報告義務・監査受入を明文化== することが基本。==定期報告と監査の組合せで実効性を担保==（ア）。\n__イ__ は ==個人ペナルティだけでは抑止力が限定的==（組織的対応が必要）。\n__ウ__ は委託元が ==指揮命令すると偽装請負のリスク==（請負契約に反する）。\n__エ__ は「==一任==」では委託元の ==管理責任を果たせない==。',
    categoryId: 'project-work',
    sourceUrl: H30_SPRING_PM_AM2_SOURCE_URL,
  },
  {
  ...
  },
  {
    ...
    correctIndex: 1,
    explanation: '==公開鍵暗号方式== では，各人が公開鍵と秘密鍵の 1 組（2 個）を保有する。\nn 人なら ==2n 個== の鍵が全体で必要となる（イ）。\n==共通鍵暗号== は通信ペアごとに 1 個必要で ==n(n−1)/2 個==（ウ）となり，人数に対し O(n^2) で増えるのに対し，==公開鍵方式は O(n) で済む== のが鍵管理上の利点。',
    categoryId: 'service-management',
    sourceUrl: H30_SPRING_PM_AM2_SOURCE_URL,
  },
  {
  ...
  },
  {
    ...
    correctIndex: 2,
    explanation: '==ローリングウェーブ計画法==（Rolling Wave Planning）は==段階的詳細化==の手法で，==近い将来の作業は詳細に==，==遠い将来の作業は上位レベルで保留== し，詳細が明確になってから要素分解する（ウ）。\n__ア__ は ==WBS 辞書== の役割（WBS 要素ごとの詳細記述）。\n__イ__ は ==テンプレート利用==。\n__エ__ は ==通常の要素分解== の説明。',
    categoryId: 'planning',
    sourceUrl: H29_SPRING_PM_AM2_SOURCE_URL,
  },
  {
  ...
  },
  {
    ...
    correctIndex: 3,
    explanation: '==派遣先責任者== は派遣先における派遣労働者の適正な就業確保のため，==指揮命令者へ関連法規・派遣契約内容・派遣元からの通知を周知== する義務を負う（エ）。\n__ア__ ==人事記録と考課== は ==派遣元事業主== の役割。\n__イ__ 派遣就業場所が複数なら ==原則として場所ごとに選任== する必要がある。\n__ウ__ ==管理職位の者から選任== する必要は法定要件ではない。',
    categoryId: 'governance',
    sourceUrl: H29_SPRING_PM_AM2_SOURCE_URL,
  },
  {
  ...
  },
  {
    ...
    correctIndex: 0,
    explanation: '==SPA==（Software Process Assessment，==ISO/IEC 15504/SPICE==）は ==組織のソフトウェアプロセスの能力水準を評価== し，==継続的改善== のために用いる国際標準（ア）。\n__イ__ は ==JIS X 0160==（ソフトウェアライフサイクルプロセス）の説明。\n__ウ__ は ==ISO 9001 系==（品質マネジメント）の説明。\n__エ__ は ==PMBOK== の説明。',
    categoryId: 'development-approach',
  ...
  },
  {
    ...
    correctIndex: 1,
    explanation: '==プロジェクトの不確実性== は ==情報が少ない開始時に最も高く==，進行とともに低下する（イ）。\n__ア__ は逆で ==ステークホルダ影響力は開始時が最大==。\n__ウ__ は要員数は ==中盤が最大== で典型的な S 字曲線を描く。\n__エ__ は逆で ==変更・エラー訂正コストは終盤が最大==（手戻り影響範囲が広いため）。',
    categoryId: 'planning',
    sourceUrl: H28_SPRING_PM_AM2_SOURCE_URL,
  },
  {
    ...
    correctIndex: 1,
    explanation: '==プロジェクトを公式に承認する文書== は ==プロジェクト憲章== で，==承認権限を持つのはプロジェクトスポンサー==（イ）。PM は憲章により ==任命される側== なので PM が承認する選択肢（ア・ウ）は不適切。\n__エ__ ==プロジェクトマネジメント計画書== は承認後に PM が作成する文書で，プロジェクト開始の公式承認文書ではない。',
    categoryId: 'integration',
    sourceUrl: H28_SPRING_PM_AM2_SOURCE_URL,
  },
  {
  ...
  },
  {
    ...
    correctIndex: 2,
    explanation: '==資源カレンダー==（Resource Calendar）は，==資源が利用可能な作業日・シフト・休暇・繁忙期を示す情報源== で，==所要期間見積もりにおいて資源確保可能日を把握== するために使う（ウ）。\n__ア__ は ==資源見積り==（必要資源の種類と量の特定）。\n__イ__ は ==RBS==（資源ブレークダウンストラクチャ）の役割。\n__エ__ は ==類推見積り==（過去類似プロジェクト情報による見積り）の活動。',
    categoryId: 'planning',
    sourceUrl: H28_SPRING_PM_AM2_SOURCE_URL,
  },
  {
  ...
  },
  {
    ...
    correctIndex: 1,
    explanation: '==デルファイ法==は，==複数の専門家から匿名・独立のアンケートを集めて要約== し，==再度アンケートを行うサイクルを反復することで意見の収束を図る== 技法（イ）。==匿名性と反復== が特徴。\n__ア__ は ==インタビュー==（一回限り）。\n__ウ__ は ==ブレーンストーミング/合議制==（一堂に会する）。\n__エ__ は ==SWOT 分析==（強み・弱み・好機・脅威の組合せ分析）の説明。',
    categoryId: 'uncertainty',
    sourceUrl: H28_SPRING_PM_AM2_SOURCE_URL,
  },
  {
  ...
  },
  {
    ...
    correctIndex: 1,
    explanation: '==クリティカルチェーン法（CCM）== では，各タスクのバッファを取り除いて ==クリティカルチェーン末尾にまとめて配置== する「==プロジェクトバッファ==」が中核（イ）。これによりタスク単位の ==サバ読みを防ぎ==，全体でのリスクを吸収する。\n__ア__ 非クリティカルチェーンには ==合流バッファ== を設定する。\n__ウ__ 各タスクごとのバッファ設定は CCM の発想と逆。\n__エ__ 前倒しはバッファを設けず逆方向の戦略で適切でない。',
    categoryId: 'planning',
    sourceUrl: H27_SPRING_PM_AM2_SOURCE_URL,
  },
  {
  ...
  },
  {
    ...
    correctIndex: 0,
    explanation: '==SPA==（Software Process Assessment）は ==ISO/IEC 15504（SPICE）== の前身で，==組織のソフトウェアプロセスの能力水準を判定== し ==継続的改善状況を評価== することを目的とする枠組み（ア）。\n__イ__ は ==SLCP（JIS X 0160）== の説明。\n__ウ__ は ==ISO 9001 系== の品質保証規格。\n__エ__ は ==PMBOK== の説明。',
    categoryId: 'tailoring-models',
  ...
  },
  {
    ...
    correctIndex: 2,
    explanation: '==ローリングウェーブ計画法== は，==近い将来の作業は詳細化==，==先の作業は上位レベルにとどめておき==，時期が近づき情報が明確化した段階で順次詳細化していく ==段階的詳細化（プログレッシブ・エラボレーション）== の手法（ウ）。\n__ア__ は ==WBS 辞書== の役割（WBS 要素ごとの詳細記述）。\n__イ__ は ==WBS テンプレート== の活用。\n__エ__ は ==WBS の最下位レベル（ワークパッケージ）定義== の説明。',
    categoryId: 'planning',
    sourceUrl: H26_SPRING_PM_AM2_SOURCE_URL,
  },
  {
  ...
  },
  {
    ...
    correctIndex: 0,
    explanation: '==EMV==（Expected Monetary Value，期待金額価値）はリスク事象が顕在化したときの ==影響金額 × 発生確率== で算出される（ア）。==リスクの定量的分析== や ==デシジョンツリー分析== で用いる代表的な指標。\n__イ__ は割り算で次元が合わず誤り。\n__ウ・エ__ は対応コストを掛けたり割ったりしており EMV の定義と異なる。',
    categoryId: 'uncertainty',
    sourceUrl: H26_SPRING_PM_AM2_SOURCE_URL,
  },
  {
  ...
  },
  {
    ...
    correctIndex: 1,
    explanation: '==SMTP サーバの第三者中継（オープンリレー）== を許可するとスパム送信や踏み台に悪用されるため，==社外発・社外宛のメールを中継しないよう設定== する（自ドメイン宛のみ受信／自ドメイン送信元のみ中継）のが SMTP 不正利用防止の基本対策（イ）。\n__ア__ は ==DNS サーバ== の対策。\n__ウ__ は ==Web サーバのディレクトリリスティング== 対策。\n__エ__ は ==Web サーバの CGI 実行制限==。',
    categoryId: 'service-management',
    sourceUrl: H26_SPRING_PM_AM2_SOURCE_URL,
  },
  {
    ...
    correctIndex: 1,
    explanation: '==SSL／TLS のクライアント認証用ディジタル証明書== は ==IC カードや USB トークンなどの耐タンパ装置に格納可能== で，==特定の PC に紐づける必要はなくユーザ単位で運用== できる（イ）。\n__ア__ サーバ証明書のコモンネームには ==FQDN（ホスト名）== を用い IP アドレスの組込みは必須ではない。\n__ウ__ SSL は ==不特定多数の利用者間の通信を保護するオープンな仕組み== で事前の利用者登録は不要。\n__エ__ 日本国内に SSL の鍵長を ==128 ビット未満に制限する法規制はない==（過去の米国輸出規制も既に撤廃済）。',
    categoryId: 'service-management',
    sourceUrl: H26_SPRING_PM_AM2_SOURCE_URL,
  },
  {
    ...
    correctIndex: 2,
    explanation: 'プロジェクトに関わる ==ステークホルダ== はプロジェクトの実施・成果に利害をもつ個人・組織で，==成果が自らの利益になる者（受益者）== と ==不利益になる者（被影響者）== の両方が存在し得る（ウ）。\n__ア__ は ==組織内外を問わずステークホルダ== となる（顧客・規制当局・地域社会等は外部）。\n__イ__ は ==直接参加せず間接的に影響する関係者== もステークホルダ。\n__エ__ は ==組織や集団としてのステークホルダ== もあり個人として特定できる必要はない。',
    categoryId: 'stakeholder',
  ...
  },
  {
    ...
    correctIndex: 1,
    explanation: 'プロジェクトが ==完了に近づくほど成果物の仕様や設計が固まり，機能の不確実性は減少== する（PMBOK のライフサイクル特性）（イ）。\n__ア__ ==要員数== は初期少・中期最大・終結減少の ==ベル型==。\n__ウ__ ==ステークホルダ影響度== は完了直前ほど低くなる（変更余地が少ない）。\n__エ__ ==変更／エラー修正の影響度== は完了に近づくほど ==手戻りコストは増加== する。',
    categoryId: 'project-work',
    sourceUrl: H25_SPRING_PM_AM2_SOURCE_URL,
  },
  {
    ...
    correctIndex: 3,
    explanation: '==プロジェクト憲章== は ==立上げプロセス群== でプロジェクトを認知・承認するために作成する文書で，==要求事項・目的・妥当性・全体スケジュール・PM と権限レベル== を記載する（エ）。\n__ア__ は ==プロジェクトマネジメント計画書==（計画プロセス群の集合体）。\n__イ__ は ==プロジェクトスコープ記述書==（境界・受入基準）。\n__ウ__ は ==WBS==（階層構造）の説明。',
    categoryId: 'integration',
    sourceUrl: H25_SPRING_PM_AM2_SOURCE_URL,
  },
  {
    ...
    correctIndex: 2,
    explanation: '==ワークパッケージ== は ==WBS の最下位レベルの要素== で，さらにスケジュール作成のために ==アクティビティ（活動）に分解== される（ウ）。\n__ア__ ==OBS との対応関係== は責任分担マトリックスで扱い WP の定義ではない。\n__イ__ ワークパッケージは ==関連する作業のまとまり== であって成果物そのものをまとめたものではない。\n__エ__ 上位の要素成果物と ==1 対 1 とは限らず== 複数のワークパッケージから構成される場合もある。',
    categoryId: 'planning',
    sourceUrl: H25_SPRING_PM_AM2_SOURCE_URL,
  },
  {
  ...
  },
  {
    ...
    choices: ['選択肢アのグラフ', '選択肢イのグラフ', '選択肢ウのグラフ', '選択肢エのグラフ'],
    correctIndex: 2,
    explanation: '__順調な状態の条件__:\n　==コスト超過しない== → ==AC ≤ EV==（実コストが完了出来高以下）\n　==納期に遅れない== → ==EV ≥ PV==（出来高が計画値以上）\n両方を満たすのは ==EV ≥ PV ≥ AC== の順序で，==アーンドバリュー（EV）が最も高く実コスト（AC）が最も低い== グラフ（ウ）が該当。\n__ア__ は AC 最大で EV ＞ PV，コスト超過＋進捗先行で条件不満足。\n__イ__ は AC ＞ PV ＞ EV でコスト超過＋進捗遅れ。\n__エ__ は PV ＞ EV ＞ AC で進捗遅れ。',
    categoryId: 'measurement',
    sourceUrl: H25_SPRING_PM_AM2_SOURCE_URL,
    figure: {
      type: 'svg',
  ...
  },
  {
    ...
    correctIndex: 1,
    explanation: '==保守性（Maintainability）== は ==故障発見後にどれだけ速く修正できるか== を示す指標で，==修正時間の合計 ÷ 修正件数==（平均修復時間 ==MTTR== 系の指標）が該当する（イ）。\n__ア__ は ==信頼性==（誤り密度）。\n__ウ__ は ==移植性==（変更行数比率）。\n__エ__ は ==機能適合性／要求変更率== に関する指標。',
    categoryId: 'delivery',
    sourceUrl: H25_SPRING_PM_AM2_SOURCE_URL,
  },
  {
  ...
  },
  {
    ...
    correctIndex: 2,
    explanation: '==SOA== はサービス間の独立性を高めるため ==疎結合（loose coupling）== で設計し，業務変更時の影響範囲を局所化する（ウ）。\n__ア__ ==ステートフル== は独立性・スケーラビリティを下げるため SOA では ==ステートレスが原則==。\n__イ__ サービス名は ==役割ではなくビジネス機能（業務内容）を表すように命名== する。\n__エ__ ==再利用性== は SOA の中核的な目的で「再利用しない方がよい」は誤り。',
    categoryId: 'development-approach',
    sourceUrl: H25_SPRING_PM_AM2_SOURCE_URL,
  },
  {
  ...
  },
  {
    ...
    correctIndex: 2,
    explanation: '==フェールソフト== は ==障害発生時に機能や性能を縮退させつつ運転を継続== させる設計思想で，「==パフォーマンス低下を許容しサービスを継続==」する事例（ウ）が該当。\n__ア・イ__ は障害発生時に ==安全な停止== を選ぶ ==フェールセーフ==。\n__エ__ は ==誤操作を未然に防ぐフールプルーフ== の説明。',
    categoryId: 'service-management',
    sourceUrl: H25_SPRING_PM_AM2_SOURCE_URL,
  },
  {
    ...
    correctIndex: 3,
    explanation: '==複数ファイルに分散したデータ== は個々のファイルを単独にバックアップすると ==ファイル間の整合性が崩れる== ため，==一連の更新処理が完了した整合性ある時点でまとめてバックアップ== するのが適切（エ）。\n__ア__ ==バックアップを消去== すると過去復旧不能になる。\n__イ__ 全データを取得しないと ==差分／増分の基準が失われ== 復旧時に整合しない。\n__ウ__ ==同一媒体上書き== は媒体障害や上書き失敗時に復旧手段を失うため不可。',
    categoryId: 'service-management',
    sourceUrl: H25_SPRING_PM_AM2_SOURCE_URL,
  },
  {
  ...
  },
  {
    ...
    correctIndex: 2,
    explanation: '==国際エネルギースターロゴ== は米国 EPA が制定した ==オフィス機器（PC・プリンタ・複合機等）の省エネ性能基準== に適合した製品に付与される環境ラベルで，==稼働時／待機時の消費電力基準== を満たすことを示す（ウ）。\n__ア__ ==FSC== は森林管理協議会の森林認証（木材製品向け）。\n__イ__ ==グリーンマーク== は古紙再生紙利用製品向け。\n__エ__ ==統一省エネラベル== は家電製品（エアコン・冷蔵庫等）の省エネ性能表示制度で情報機器向けではない。',
    categoryId: 'governance',
    sourceUrl: H25_SPRING_PM_AM2_SOURCE_URL,
  },
  {
    ...
    choices: ['ISO 14001', 'RoHS 指令', 'WEEE 指令', 'グリーン購入法'],
    correctIndex: 1,
    explanation: '==RoHS 指令==（Restriction of Hazardous Substances）は ==EU が 2006 年に施行== した指令で，==電気・電子機器における鉛・水銀・カドミウム・六価クロム・PBB・PBDE 等の特定有害物質== の使用を基準値以下に制限する（イ）。\n__ア__ ==ISO 14001== は環境マネジメントシステムの国際規格で物質規制ではない。\n__ウ__ ==WEEE 指令== は廃電気電子機器のリサイクル責任を規定する別指令。\n__エ__ ==グリーン購入法== は日本の法律で公的機関の環境物品調達促進が主目的。',
    categoryId: 'governance',
    sourceUrl: H25_SPRING_PM_AM2_SOURCE_URL,
  },
  {
    ...
    correctIndex: 2,
    explanation: '==個人情報保護法== における個人情報は「==生存する個人に関する情報であって，特定の個人を識別できるもの==」と定義され，==亡くなった個人の情報は原則として対象外==（ウ）。\n__ア__ ==企業の顧客に限らず== 従業員・取引先個人なども含む。\n__イ__ ==秘密にしているプライバシー情報に限らず== 公知情報でも識別可能なら個人情報。\n__エ__ ==国籍は問わず== 日本国内に居住する外国人も保護対象に含まれる。',
    categoryId: 'governance',
    sourceUrl: H25_SPRING_PM_AM2_SOURCE_URL,
  },
  {
