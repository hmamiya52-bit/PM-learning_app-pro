...
export const essayProblems: EssayProblem[] = [
  ...
  },
  {
    ...
    preamble: [
      ...
      '　プロジェクトの実行中に察知した兆候としては，例えば，メンバの稼働時間が計画以上に増加している状況や，メンバが仕様書の記述に対して分かりにくさを表明している状況などが挙げられる。これらの兆候をそのままにしておくと，開発生産性が目標に達しないリスクや成果物の品質を確保できないリスクなどが顕在化し，プロジェクト目標の達成を阻害するおそれがある。',
  ...
  },
  {
    ...
    preamble: [
      '　プロジェクトマネージャ（PM）には，開発する情報システムの品質を適切に管理することが求められる。このため，プロジェクトの目標や特徴を考慮して，開発工程ごとに設計書やプログラムなどの成果物の品質に対する評価指標，評価指標値の目標範囲などを定めて，成果物の品質を評価することが必要になる。',
      '　プロジェクト推進中は，定めた評価指標の実績値によって成果物の品質を評価する。特に，実績値が目標範囲を逸脱しているときは，その原因を分析して特定する必要がある。例えば，設計工程において，ある設計書のレビュー指摘密度が目標範囲を上回っているとき，指摘内容を確認すると，要件との不一致に関する指摘事項が多かった。その原因を分析して，要件定義書の記述に難解な点があるという原因を特定した，などである。また，特定した原因による他の成果物への波及の有無などの影響についても分析しておく必要がある。',
  ...
  },
  {
    ...
    preamble: [
      ...
      '　工数の見積りは，見積りを行う時点までに入手した情報とその精度などの特徴を踏まえて，開発規模と生産性からトップダウンで行ったり，WBSの各アクティビティをベースにボトムアップで行ったり，それらを組み合わせて行ったりする。PMは，所属する組織で使われている機能別やアクティビティ別の生産性の基準値，類似プロジェクトの経験値，調査機関が公表している調査結果などを用い，使用する開発技術，品質目標，スケジュール，組織要員体制などのプロジェクトの特徴を考慮して工数を見積もる。未経験の開発技術を使う場合，経験値の入手が困難な場合は，システムの一部分を先行開発して関係する係数を実測するなど，見積りをできるだけ正確に行うための工夫を行う。',
      '　見積りどおりに工数をコントロールするためには，プロジェクト運営面で様々な施策が必要となる。PMは，システム開発標準の整備と周知徹底，要員への適正な作業割当てなどによって，工数当たりの生産性を維持することに努めなければならない。また，プロジェクトの進捗に応じた工数の実績と見積りの差異や，開発規模や生産性に関わる見積りの前提条件の変更内容などを常に把握して，プロジェクトのコストや進捗に影響を与える問題を早期に発見して，必要な対策を行うことが重要である。',
      '　あなたの経験と考えに基づいて，設問ア～ウに従って論述せよ。',
    ].join('\n'),
    setsumons: [
      {
  ...
  },
  {
    ...
    preamble: [
      ...
      '　プロジェクト目標の達成は，要員に期待した能力が十分に発揮されるかどうかに依存することが少なくない。プロジェクト組織体制の中で，要員に期待した能力が十分に発揮されない事態になると，担当させた作業が目標の期間で完了できなかったり，目標とする品質を満足できなかったりするなど，プロジェクト目標の達成にまで影響が及ぶことがある。',
  ...
  },
  {
    ...
    preamble: [
      ...
      '　PMは，特定したリスクを分析評価した上で，リスクに対応するために，技術面の予防策だけでなく運営面の予防策も立案する。運営面の予防策では，個人情報の取扱時の役割分担や管理ルールを定めたり，財務情報の参照時の承認手続や作業手順を定めたりする。立案した予防策は，メンバに周知する。',
  ...
  },
  {
    ...
    preamble: [
      ...
      '　プロジェクトの遂行中に，例えば，プロジェクトの納期を守れなくなる問題が発生したとき，この問題の解決に際し，制約条件である納期を満足させようとすれば予算超過となり，もう一つの制約条件である予算を満足させようとすれば納期遅延となる場合，納期と予算のトレードオフとなる。この場合，制約条件である納期について分析したり，その他の条件も考慮に入れたりしながら調整し，トレードオフになった納期と予算が同時に受け入れられる状態を探すこと，すなわちトレードオフを解消することが必要になる。',
      '　あなたの経験と考えに基づいて，設問ア～ウに従って論述せよ。',
    ].join('\n'),
    setsumons: [
      {
