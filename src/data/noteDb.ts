...
export const NOTE_DB: Record<string, NoteData> = {
  ...
  delivery: {
    ...
    sections: [
      ...
      {
        heading: '8. 品質保証（QA）と品質管理（QC）の違い',
        items: [
          ...
          '　　PMBOK第6版では ==8.3 品質コントロール==（==監視・コントロールプロセス群==）。技法と成果物は §9 に詳しい',
      ...
      },
      {
        ...
        items: [
          ...
          '==品質メトリクス==（IPA は ==品質尺度==）: 品質を定量測定する基準。例: ==欠陥密度==（バグ数／KLOC）／==テストカバレッジ==／==MTBF==（平均故障間隔）／==MTTR==（平均修復時間）',
        ],
      },
      {
      ...
      },
      {
        heading: '14. 新7QC道具',
        items: [
          '==新7QC道具==: __言語データ__（意見・要望・課題の文章）を整理する7つの図。数値を扱う7QC道具（§13）と対をなす',
          '__ここで問われるのは対比ひとつだけ__: 7QC道具は==数値データ==、新7QC道具は==言語データ==',
          '__押さえるのは3つで足りる__:',
          '　==親和図==: バラバラな意見を__似たもの同士でグループ化__する（KJ法とほぼ同じ）',
          '　==連関図==: 原因が絡み合って一本道にならないとき、__要素間の因果を矢印で結んで__主要因を見つける',
          '　==系統図==: 目的を__手段へ枝分かれさせて__具体策まで落とす',
          '残る4つ（マトリクス図・マトリクスデータ解析・PDPC・アローダイアグラム）は__名前だけ__でよい',
        ],
        navyItems: [[{ text: '新7QC道具は12年度300問で単独出題0回。深入りせず「言語データ側」という位置づけだけ押さえる', style: 'navy' }]],
      },
      {
  ...
  measurement: {
    ...
    sections: [
      // ── A. 領域概観 ──
      {
        ...
        items: [
          'PMBOK第7版「==測定==」パフォーマンス領域: パフォーマンスを監視・評価し適切な対応を取る活動。第6版では第7章コスト（EVM 中心）が対応',
          '__測定は手段__: 数字を集めること自体が目的ではなく、==意思決定==のために測る。「この数字で何を判断するか」を決めてから指標を選ぶ',
        ],
      },
      {
        heading: '2. メトリクスの分類',
        items: [
          '==リーディング指標==（先行指標）: 結果が出る__前__に動く指標。まだ手を打てるのが利点（例: レビュー指摘密度・要員稼働率）',
          '==ラギング指標==（遅行指標）: 結果が__確定した後__の指標。正確だが対策が間に合わない（例: 売上・最終欠陥数）',
          '==成果メトリクス==（結果に焦点）／==実行メトリクス==（活動量に焦点）の区別も問われる',
          '__使い分け__: 事後評価はラギング、__早期に手を打ちたい__ならリーディングを併用する',
        ],
      },
      {
        ...
        items: [
          ...
          '__どちらを置くか__: __達成度__を評価したいなら KPI、__予兆__を検知して先に手を打ちたいなら KRI',
          '　KRI は==しきい値==とセットで設計する（超えたら対応を発動 → §25 定石8）。しきい値のない KRI は見逃す',
      ...
      },
      {
        ...
        items: [
          // 赤字密度メモ: EVM 差異は午前Ⅱ最頻出のため 7 個許容（方針書 §7）
          '__差異分析__の2指標（単位はコスト。__プラスが良い__）:',
          '　==CV==（Cost Variance, コスト差異）= ==EV - AC==',
          '　　CV > 0 なら==コスト節約==／CV < 0 なら超過',
          '　==SV==（Schedule Variance, スケジュール差異）= ==EV - PV==',
          '　　SV > 0 なら==スケジュール先行==／SV < 0 なら遅延',
          '__覚え方の型__: どちらも「==EV から引く==」。引く相手が AC ならコスト、PV ならスケジュール',
          '__健全性の4象限__: CV・SV とも正なら順調／CV正・SV負は「安いが遅い」／CV負・SV正は「速いが高い」／CV・SV とも負は危機的',
        ],
      },
      {
        heading: '6. パフォーマンス効率指数（CPI / SPI）',
        items: [
          // 赤字密度メモ: EVM 効率指数は午前Ⅱ最頻出のため 9 個許容（方針書 §7）
          '__効率指数__の2指標（比率・無次元。__1を境に判定__）:',
          '　==CPI==（Cost Performance Index, コスト効率指数）= ==EV / AC==',
          '　　==CPI > 1== ならコスト効率良好／1未満なら悪化',
          '　==SPI==（Schedule Performance Index, スケジュール効率指数）= ==EV / PV==',
          '　　==SPI > 1== ならスケジュール先行／1未満なら遅延',
          '__差異（CV/SV）でなく指数を使う理由__: 比率なので==規模に依存しない==。工程間・チーム間・案件間を同じ物差しで比べられる',
          '__SPI の限界__: 終盤は EV も PV も BAC に近づくため、遅れていても==必ず1に収束==する。終盤の遅延判定には ==アーンドスケジュール（ES）== を使う（§9）',
        ],
      ...
      // ── C. スケジュール・進捗・規模の測定 ──
      {
        ...
        items: [
          ...
          '==バーンアップ・チャート==: __完了量__と__総スコープ__の2本線。総スコープ線が動くため==スコープ変更==が見える',
      ...
      },
      {
        ...
        items: [
          '==リードタイム==: 顧客が要求してから受領するまで（顧客視点）／==サイクルタイム==: 作業に着手してから完了まで（開発視点）',
          '　リードタイムは__着手待ちを含む__。サイクルタイムが同じでも、待ち行列が長ければリードタイムは伸びる',
          '==スループット==: 単位時間あたりの完了量',
          '==リトルの法則==: ==WIP = スループット × サイクルタイム==',
          '　__使いどころ__: スループットが一定なら==WIP を減らすとサイクルタイムが縮む==。仕掛かりを絞る根拠になる',
        ],
        navyItems: [[{ text: '本ノートは「WIP = スループット × サイクルタイム」で統一する（サイクルタイムは着手〜完了）', style: 'navy' }]],
      },
      {
        heading: '13. 規模・工数の見積もり（FP法・COCOMO・ベロシティ）',
        items: [
      ...
      // ── E. 戦略的測定 ──
      {
        ...
        items: [
          '==BSC==（Balanced Scorecard, バランススコアカード）: 戦略実行のための多次元評価フレームワーク（Kaplan & Norton, 1992年）',
          '__4つの視点__（下が土台、上が結果）:',
          '　==財務の視点==（Financial）: 株主への価値（売上・利益・ROE）',
          '　==顧客の視点==（Customer）: 顧客満足・市場シェア・顧客維持率',
          '　==業務プロセスの視点==（Internal Process）: 業務効率・品質・革新',
          '　==学習と成長の視点==（Learning & Growth）: 従業員スキル・組織能力・情報技術',
          '__順番で覚える__: 人が育つ（学習と成長）→ 業務が回る（プロセス）→ 顧客が満足する → __財務に表れる__。この==因果関係==を矢印で図示したものが__戦略マップ__',
        ],
      },
      {
      ...
      // ── F. パフォーマンス・レポート ──
      {
        heading: '20. 報告書の種類',
        items: [
          '報告書は__答える問いの違い__で4種類に分かれる。詳細は project-work §18',
          '　==ステータス==: 今どうなっているか（現状）',
          '　==トレンド==: どちらへ向かっているか（傾向）',
