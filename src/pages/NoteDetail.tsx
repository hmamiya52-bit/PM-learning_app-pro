  ...
  stakeholder: {
    ...
    sections: [
      // ── A. 定義・基本概念 ──
      {
        ...
        items: [
          'プロジェクトの結果に影響を与える、または影響を受ける可能性のある==個人==または==組織==の総称',
          'PMBOK第7版では==ステークホルダー・パフォーマンス領域==として独立した章で扱われる（PMBOK7th p.8, p.10）',
          'ステークホルダーは==プロジェクトのライフサイクル全体==で変動するため、継続的な再識別が必要',
          '効果的なステークホルダー関与は==プロジェクト成功確率==を直接的に高める',
          '個人だけでなくグループ・部門・組織・規制当局など==法人格を持たない集団==も含む',
        ],
      ...
      // ── B. 識別プロセス ──
      {
        ...
        items: [
          '識別は==プロジェクト開始時==の1回だけでは不十分',
          'プロジェクトのライフサイクル全体を通じて==反復的==に実施する',
          '識別のトリガー: 体制変更・スコープ変更・フェーズゲート・課題発生時',
          '適応型ライフサイクル（アジャイル）では==各イテレーション開始時==に再識別',
          '識別漏れは==プロジェクト後半での要件追加==・受入拒否のリスクを高める',
        ],
      },
      {
        heading: '8. 識別の技法',
      ...
      },
      {
        ...
        items: [
          '識別漏れの典型例: ==間接的影響者==（地域住民・労組）、==規制当局==、==退職予定の現業ユーザ==',
          'リスク: プロジェクト終盤の==スコープ変更要求==・==受入拒否==・==訴訟==',
          '対策1: ==複数視点==で識別する（PM・スポンサー・ベテラン・現場の4視点）',
          '対策2: ==類似プロジェクトの教訓==を必ず参照する',
          '対策3: ==フェーズゲート==で識別の妥当性をレビューする',
      ...
      // ── C. 分析・優先順位付け ──
      {
        ...
        items: [
          ...
          '分析手法は==単一の正解はない==（プロジェクト特性に応じて選択）',
          '複数手法を組み合わせて==多角的に分析==することが推奨される',
        ],
      },
      {
      ...
      },
      {
        ...
        items: [
          ...
          'ステークホルダーが==プロジェクト変更にどれだけ影響==されるかを評価する観点',
      ...
      },
      {
        ...
        items: [
          '==3つの属性==で分析: Power・Legitimacy・Urgency',
          '==Power（権力）==: 自分の意思を相手に実行させる能力',
          '==Legitimacy（合法性／正当性）==: 関与が==社会的に妥当==と認められているか',
          '==Urgency（緊急性）==: 即時の対応を要求する度合い',
          '3属性のうち==いくつ持つか==でステークホルダーを分類（1属性=Latent、2属性=Expectant、3属性=Definitive）',
          '権力だけでなく==正当性・緊急性==を加えた多次元分析が特徴',
        ],
      ...
      // ── D. エンゲージメント計画 ──
      {
        ...
        items: [
          ...
          '==C と D が一致==している場合、追加施策不要',
          '==C と D が異なる==場合、ギャップを埋める関与戦略を立案',
      ...
      },
      {
        ...
        items: [
          '==スポンサー==: 定期的な1on1、重要決定の事前合意、エスカレーション窓口の確立',
          '==顧客／エンドユーザ==: 受入基準の合意、定期デモ、UATへの早期巻込み',
          '==機能部門マネジャー==: リソース調整、優先順位の明確化',
          '==チームメンバ==: 役割明確化、定期1on1、心理的安全性の確保',
          '==規制当局==: 早期確認、文書化された承認の取得',
          '==サプライヤ==: 契約条件の明確化、パフォーマンス・レビュー',
        ],
      },
      {
        heading: '23. 文化的・組織的多様性への配慮',
        items: [
          '==言語==: 母国語の違いによる誤解・専門用語の翻訳問題',
          '==文化==: 直接的/間接的コミュニケーション、合意形成プロセスの違い',
          '==タイムゾーン==: 会議時間・レスポンス期待値の調整',
          '==階層意識==: 国・組織による上下関係の強さの違い',
          '==意思決定スタイル==: トップダウン/ボトムアップ、コンセンサス重視',
      ...
      },
      {
        ...
        items: [
          '==定例会議==: 進捗報告（週次・隔週）、ステアリングコミッティ（月次）',
          '==アドホック会議==: 課題対応・意思決定・キックオフ',
          '==報告書==: ステータスレポート、トレンドレポート、予測レポート、バリアンスレポート',
          '会議体の設計原則: ==目的==・==参加者==・==時間==・==アジェンダ==を事前定義',
          '==アクションアイテム==は責任者・期限とともに記録し、次回会議で進捗確認',
        ],
      },
      {
        ...
        items: [
          ...
          '基準はプロジェクト開始時に==合意・文書化==しておく',
          'ステークホルダーの==過剰なエスカレーション==は信頼を損なうため、基準遵守が重要',
        ],
      },
      {
      ...
      // ── F. 監視・コントロール ──
      {
        ...
        items: [
          '監視の目的: 関与レベルが==計画通りか==、変化があれば早期検知',
          '==KPI 例==: 会議出席率、レビュー反応時間、アンケートスコア',
          '==指標==: ステークホルダー満足度、変更要求件数、エスカレーション件数',
          '監視は==定量==（数値）と==定性==（観察・対話）の両面で実施',
      ...
      },
      {
        ...
        items: [
          '関与レベル低下の典型シグナル: ==会議欠席==、==レビュー遅延==、==非協力的態度==',
          '対応1: ==1on1==で個別ヒアリング、懸念の把握',
          '対応2: ==コミュニケーション頻度・内容==の見直し',
          '対応3: ==計画への参加==機会を提供（共同設計）',
          '対応4: ==スポンサー経由==での働きかけ',
      ...
      },
      {
        ...
        items: [
          '==Influence vs Impact==: 「影響力」と「影響度」を取り違える誤答選択肢に注意',
          '==Power vs Influence==: 公式権威（Power）と非公式影響力（Influence）の区別',
          '==関与レベルの段階==: 「中立」と「支持」、「抵抗」と「不認識」の境界を問う設問',
          '==登録簿 vs 計画書==: ステークホルダー登録簿（識別アウトプット）と==エンゲージメント計画書==（戦略文書）の混同',
          '==プッシュ／プル／インタラクティブ==: それぞれの適用シーンの誤認',
          '==サリエンスモデルの属性数==: 1属性のみ（Latent）と2属性（Expectant）の分類混同',
