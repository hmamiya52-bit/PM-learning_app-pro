...
- `overview` は**「状況 → ぶつかる難所 → 何が問われるか」の流れ**で書く。問われる設問を平板に列挙するのでなく、PMが何に直面しどう動くかの物語にする。「〜を組み立てる／整理する」等の便利動詞でぼかさず具体の動作で書き、初出の専門語（実績照会機能 等）は overview ではかみ砕くか避ける（H26セルフレビューの教訓）。
