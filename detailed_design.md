...
> バージョン: 0.24（F2-P8 を Claude 単独と確定 / 画像PDF=PyMuPDF精読手順を §2.7g.2 に追記）
> 作成日: 2026-05-06（v0.14） / 2026-05-13（v0.15） / 2026-05-16（v0.16） / 2026-05-17（v0.17・v0.18・v0.19・v0.20・v0.21） / 2026-05-28（v0.22） / 2026-05-29（v0.23・v0.24）
...
| **F2-P8** | 午後I 独自解説（★v0.23新設・F2-P7より先行） | 🅒（v0.24で単独確定） | C: 20〜30h / 経過: 4〜6d | パイロットR6/R5/R4の解説完成＋checkMode統合UI + 検証pass |
| **F2-P9** | 午後II 参考答案（★v0.23新設） | 🅒+Codexレビュー | C: 25〜40h / 経過: 5〜8d | パイロットR6/R5(4本)＋リビールUI + ユーザ妥当性確認 |

**フェーズ2合計（Claude時間ベース）**: 約 **160〜245h** / 経過日数 **約27〜40日**（F2-P8/P9 を除く。両者は別途 +45〜70h）

> **着手順（v0.23・2026-05-29 確定）**: **F2-P8（午後I解説）を F2-P7 より先行**。最大の学習ギャップ（午後I解説が皆無）を先に埋める。詳細は [§2.7g](#27g-午後i解説--午後ii参考答案v023新設) および `docs/afternoon_explanation_design.md`。
...
#### F2-P6 PMBOK第8版要素 12カテゴリ統合（0.5日）  🅒
- **v0.22 修正（2026-05-28）**: 独立カテゴリ `pmbok8-diff` 新設方針を撤回し、12カテゴリへの注記統合に変更
- **修正理由**:
  - 実データ分析（午前II 300問・午後I 37問・午後II 24問）で第8版固有用語は ≒ 0%（4/361件のみ、しかも H28-R6 にかけて散発、AI/ML/デザイン思考は第6版時代から登場）
  - PMBOK 第8版は 2026-04 日本語版リリース、PMP 試験反映は 2026-07-09。IPA 試験要綱の反映は早くても 2027-2028 春期試験から
  - 第8版の主変更（12原則→6原則、8領域→7領域）は**第7版の再編・統合**で根本的新概念は少ない
  - 設計書 §PMBOK版バランス方針（v0.18）「第6版が出題中核」と独立カテゴリ新設は整合しない
  - 「闇雲に量を増やす」リスクの回避を優先
- **新方針**: 各カテゴリ末尾に「N. PMBOK第8版での位置づけ」セクションを 1 つ追加（100-200字、navyItems で補足）
- **マッピング設計**: `docs/pmbok_v7_to_v8_mapping.md` を参照
- **クイズ追加なし**: 試験未反映のため
- 成果物: 12カテゴリへ統合注記 + マッピング設計書
...
- **マークアップ整合性チェック必須**（v0.19 追加）:
  ...
  - 全角イコール `＝` の混入なし
  - `===` / `___` / `====` / `____` のような連続記号なし
- **インデント整合性チェック必須**（v0.20 追加）:
  - 包含関係のある列挙項目は **全角スペース `　`** で階層化（最大2階層）
  - 「親見出し:」直後の列挙項目にインデントが付いている
  - 番号付きステップは同一階層で揃っている
  - 詳細は `docs/note_markup_rules.md` §5
...
## 2.7g 午後I解説 / 午後II参考答案（v0.23新設）

> 詳細仕様の正本は `docs/afternoon_explanation_design.md`（方針確定版＋2026-05-29 第2回細部確定）。本節は detailed_design 上のフェーズ位置づけと要点のみ。

### 2.7g.1 背景・スコープ整合
- 午後I: `officialAnswers.ts` に IPA 公式「解答例」はあるが**解説が皆無** → 自己採点後に「なぜその解答か／本文のどこが根拠か」が学べない最大の学習ギャップ。
- 午後II: `essayProblems.ts` にテーマ・設問のみで**参考答案が皆無** → 5軸自己評価の答え合わせ材料が無い。
- 著作権: IPA は午後I「解説」・午後II「模範解答」を非公開 → どちらも 100% Claude 独自著作。本文の逐語引用は禁止（位置参照＋言い換え）。
- スコープ: 参考答案は「採点機能」ではなく「参考読み物」→ `project_scope.md` の論述採点非対応に抵触しない。UI で「論述例の一つ」と明示。

### 2.7g.2 F2-P8 午後I 独自解説（F2-P7 より先行）  🅒+Codexレビュー
- **データ**: 新規 `src/data/afternoonExplanations.ts`（IPA引用の `officialAnswers.ts` は汚さない）。型 `AfternoonExplanation`（`overview` ＋ `rows: AfternoonRowExplanation[]`）。`rowKey = ${s}|${q ?? ''}|${t ?? ''}` で突合。
- **粒度**: 全解答行に `point/basis/reasoning`、`pitfall` は難所のみ任意（2026-05-29 決定）。
- **執筆**: Claude が問題本文 PDF を精読し根拠付き（委譲不可）。
- **担当（v0.24・2026-05-29 確定）**: **F2-P8 は Claude 単独（🅒）**。コンテンツ純度が高く機械作業が極小（〜5%）で、ターン制では分担しても並列化されずトークン節約にならない（むしろ指示書＋レビューの調整コストが上回る）ため。F2-P1〜P4 のような大量OCR/整形が無い領域。
- **PDF 取得**: IPA 午後I 問題PDFは**画像ベース（テキスト抽出不可）**。PyMuPDF でページPNG化 → マルチモーダルで精読（150DPIで判読可を実機確認済）。著作権上 IPA ページ画像はリポジトリに**コミットしない**（毎回 ephemeral 生成）。
- **UI**: `AfternoonMyAnswer.tsx` の checkMode に行解説アコーディオン（`<details>`、**デフォルト折りたたみ**）＋ overview を統合。`AfternoonAnswerDetail.tsx` には当面追加しない。
- **段階投入**: パイロット R6/R5/R4（約22問）→ ユーザ価値検証 → 残り15問。
- **品質ゲート**: §2.7e.3 共通（validate-data / build / マークアップ整合）＋ 本文逐語引用なしの著作権チェック＋ `rowKey` 欠落検証。

### 2.7g.3 F2-P9 午後II 参考答案  🅒+Codexレビュー
- **データ**: 新規 `src/data/essaySampleAnswers.ts`。型 `EssaySampleAnswer`（`byLabel`／`designNote`／`pitfalls`）。フル参考答案 1テーマ1本＝最大24本。
- **字数**: 推奨範囲のやや上限寄り（ア≒750/イ≒1400/ウ≒1100字目安）。
- **framing**: UIラベル「参考答案」、冒頭に「これは論述例の一つです。唯一の正解ではありません」。5軸自己評価には介入しない。
- **UI**: `EssayAttemptDetail.tsx`（提出後の履歴詳細）に「参考答案を見る」リビール。**書く前は非表示**（コピー防止）。自分の答案と並置/切替。
- **段階投入・最大の品質リスク**: フル答案24本≒48,000字。パイロット R6/R5（4本）→ ユーザ妥当性確認ゲート → 残り20本。着手時に §0.2 の細部（文体・事例・designNote・注記）を再確認。
...
- F2-P7-S2: QAチェックリスト全項目消化（1日）
- F2-P7-S3: v1.0.0 リリース（0.5日）
- F2-P8: 午後I 独自解説（F2-P7 より先行・v0.23、詳細 §2.7g）
- F2-P9: 午後II 参考答案（v0.23、詳細 §2.7g）
...
## 変更履歴
- 2026-05-29 v0.24 F2-P8 担当確定:
  - F2-P8 を **Claude 単独（🅒）** と確定（コンテンツ純度高・機械作業極小・ターン制で分担効果なしのため）。
  - §2.7g.2 に画像ベースPDFの PyMuPDF 精読手順・著作権注記（IPA画像は非コミット）を追記。
- 2026-05-29 v0.23 午後I解説/午後II参考答案 要件確定反映:
  - §2.7g 新設（F2-P8 午後I独自解説 / F2-P9 午後II参考答案）。F2-P8 を F2-P7 より先行と確定。
  - フェーズ表（§1.x）に F2-P8/P9 行追加、§2.8 概要に追記。
  - 細部4点をユーザ合意（粒度=全行＋pitfall難所のみ / 動線=AfternoonMyAnswer checkMode統合・折りたたみ / 午後II字数=やや上限寄り / F2-P8先行）。
  - 詳細仕様の正本は `docs/afternoon_explanation_design.md`（§0.1/§0.2 追記）。
