...
> バージョン: 0.14（3視点レビュー反映・フェーズ1.5パイロット追加・工数細分化・Go/No-Go判定）
> 作成日: 2026-05-06
> 関連: [requirements.md v0.4](./requirements.md) / [basic_design.md v0.4](./basic_design.md)
...
| **F1.5-P2** | パイロットノート作成（1カテゴリ） | 🅒+Codex+Geminiレビュー | C: 8h + レビュー4h / 経過: 1.5d | 30セクション以上 + Codex/Geminiレビュー2巡完了 |
| **F1.5-P3** | パイロットクイズ作成（50問） | 🅒+Codex+Geminiレビュー | C: 6h + レビュー3h / 経過: 1d | 50問 + 全choices正解含む + 解説あり |
...
|---|---|---|---|---|
| **F2-P1** | 残り11カテゴリのノート | 🅒+Codex+Geminiレビュー | C: 50〜80h / 経過: 7〜12d | 11カテゴリ全て + 検証スクリプトpass |
| **F2-P2** | 残り11カテゴリのクイズ（550問） | 🅒+Codex+Geminiレビュー | C: 30〜50h / 経過: 5〜8d | 各カテゴリ50問 + 検証pass |
...
| **F2-P6** | PMBOK第8版差分カテゴリ（pmbok8-diff） ★v0.14新設 | 🅒+Codex+Geminiレビュー | C: 8h / 経過: 1.5d | 差分ノート完成 |
...
> 工数は**Claudeセッションでアプリを操作する時間ベース**。コンテンツレビュー時間（Gemini/Codex/ユーザ）は別途加算。
...
- フロー:
  ...
  3. **Gemini レビュー**（コンテンツ的妥当性、PMBOKとの整合性、表現の自然さ）
  4. **ユーザ最終確認** → 修正反映
- 完了条件: §5.9 検証スクリプト pass + 上記レビュー2巡完了
...
- フロー:
  ...
  3. **Gemini レビュー**（誤答の妥当性、難易度バランス、解説の正確性）
  4. **ユーザ最終確認**
- 完了条件: 50問+検証pass
...
### 2.7b.5 担当
- F1.5-P1: ユーザ
- F1.5-P2/P3/P4: 🅒（生成）+ Codex/Gemini（レビュー）
- F1.5-P5: ユーザ
- F1.5-P6: 🅒🅧
...
## 2.8 フェーズ2 タスク概要（D6で詳細化）

フェーズ2はコンテンツ作成中心のため、D1では概要のみ示し、詳細はD6で扱う。

### F2-P1: ノート本文作成（4〜7日）  🅒
- 12カテゴリ×30〜100セクションのノート本文を**Claude生成**
- NoteData / NoteSection / RichProtocolTable等の構造化データに落とし込む
- 赤字キーワード抽出
- ユーザレビュー → フィードバック → 修正
> Codexには不向き（PMBOK知識・文体判断・構造化センスが必要）

### F2-P2: クイズ問題作成（5〜10日）  🅒
- ノート連動の4択／記述問題を**Claude生成**
- カテゴリ毎50問程度、難易度1〜3を散らす
- 正解 + 誤答3つ（迷いやすい誤答）
- 解説テキスト
> 誤答生成の質が学習効果に直結するためClaude担当

### F2-P3: 公式午前II 問題データ＋独自解説（4〜7日）  🅒🅧
- **OCR（問題文・選択肢）**: 🅒（精度重要、IPA PDF→TS構造体）
- **独自解説生成**: 🅒（PM試験の知識が必要）
- **TS整形（OCR結果→構造化）**: 🅧（mechanical変換）
- 全年度（H25〜現行）×25問 = 約325問
- カテゴリタグ付け: 🅒

### F2-P4: 午後I 過去問＋公式解答＋配点マップ（3〜5日）  🅒🅧
- AfternoonProblem インデックス（H25〜）: 🅧（PDFリンク列挙）
- OfficialAnswerSet テキスト化（IPA PDF→TS）: 🅒🅧（OCRはClaude、整形はCodex）
- 配点マップ（NWルール踏襲、`memory/phase2_content_creation.md` 参照）: 🅧（ルール機械適用）

### F2-P5: 論述問題インデックス（午後II）（1日）  🅒🅧
- EssayProblem 一覧（H25〜）: 🅧（年度・問番号・テーマ列挙）
- 設問ごとの推奨字数を IPA公式の指示文から抽出: 🅒（OCR・読み取り）

### F2-P6: バッジ最終調整・QA消化・本番リリース（2日）  🅒🅧
- badges.ts に論述系バッジを新規追加: 🅒（条件設計）
  - 論述初回完了 / 論述継続（5/10/30回）/ 全カテゴリ論述網羅 / 重要マーク活用
- しきい値の最終調整: 🅒
- D6で作成するQAチェックリストを全項目目視で消化: 🅧（実行）→ 🅒（不具合判定）
- 致命バグの修正: 🅒
- `vercel --prod` で本番リリース: 🅒
...
const KEYS = {
  ...
  USER_PROGRESS_LEGACY: 'pmap:user_progress',  // 旧スキーマ自動破棄用
...
NW踏襲。LocalStorageキーを `nwsp:tracker:*` → `pmap:tracker:*` に置換するのみ。
...
const KEYS = {
  ...
  TRACKER_PLANS: 'pmap:tracker:plans',
